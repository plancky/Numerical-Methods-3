...
def MyTrap(func,a,b,n,d=None):
    if d is not None:
        ...
        print("Could not reach desired accuracy with the given upperlimit on number of intervals. ")
        return I[-1],n_array[-1]
    x = np.linspace(a,b,n+1)
    y = func(x)
    return((b-a)/2*np.sum(y[:-1]+y[1:]))

def MySimp(func,a,b,n,d):
    if d is not None:
        max_n = np.floor(np.log2(n))
        n_array = np.logspace(1,max_n,base=2,num = int(max_n))
        I = np.zeros(n_array.shape)
    ...
    x = np.linspace(a,b,n+1)
    y = func(x)
    return((b-a)/3*np.sum(y[::2]+2*y[1::2]+y[2::2]))
...
if __name__=="__main__":
    print(simpsons_rule(1,2,lambda x: x**2),quad(lambda x: x**2,1,2))